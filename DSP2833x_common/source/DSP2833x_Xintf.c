...
void InitXintf(void)
{
    ...
    EALLOW;
    XintfRegs.XINTCNF2.bit.XTIMCLK = 1;//1;	//修改为XTIMCLK = SYSCLKOUT/2
    // No write buffering
    XintfRegs.XINTCNF2.bit.WRBUFF = 3;	//修改为3个写缓冲，这样能加速运行
    // XCLKOUT is enabled
    XintfRegs.XINTCNF2.bit.CLKOFF = 1;  //修改为Disable XCLKOUT
    // XCLKOUT = XTIMCLK/2
    ...
    // Zone write timing
    XintfRegs.XTIMING0.bit.XWRLEAD = 1;//3;			//修改为0等待写访问建立周期
    XintfRegs.XTIMING0.bit.XWRACTIVE = 1;//7;		//修改为0等待写访问激活周期
    XintfRegs.XTIMING0.bit.XWRTRAIL = 1;//3;		//修改为0等待写访问等待周期
    // Zone read timing
    XintfRegs.XTIMING0.bit.XRDLEAD = 1;//3;			//修改为0等待读访问建立周期
    XintfRegs.XTIMING0.bit.XRDACTIVE = 1;//7;       //修改为0等待读访问激活周期
    XintfRegs.XTIMING0.bit.XRDTRAIL = 1;//3;        //修改为0等待读访问等待周期

    // double all Zone read/write lead/active/trail timing
    XintfRegs.XTIMING0.bit.X2TIMING = 0;//1;		//相对于外设的时钟比例为1:1

    // Zone will sample XREADY signal
    XintfRegs.XTIMING0.bit.USEREADY = 0;		//忽略READY信号
    ...
    XintfRegs.XTIMING7.bit.XWRACTIVE = 3;//7;
    XintfRegs.XTIMING7.bit.XWRTRAIL = 3;
    // Zone read timing
    XintfRegs.XTIMING7.bit.XRDLEAD = 3;
    XintfRegs.XTIMING7.bit.XRDACTIVE = 3;//7;
    ...
    XintfRegs.XBANK.bit.BANK = 7;			//修改为区域0
    XintfRegs.XBANK.bit.BCYC = 3;//7;			//修改为不增加时间延时
    EDIS;
